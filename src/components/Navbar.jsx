        ...
        <>
            ...
            <div className='flex gap-6 mt-8 px-4 overflow-x-auto no-scrollbar justify-center'>
                {avatarr.map((item, index) => (
                    <div key={index} className='shrink'>
                        <img className='rounded-full p-1 w-[70px] h-[70px] md:w-[95px] md:h-[95px] border-4 border-red-600 cursor-pointer'
                            src={item.img} alt="" />
            ...
            {posts.map(post =>
                <div key={post.id} className='mt-8'>
                    ...
                    <div onDoubleClick={() => toggleLike(post.id)} className='w-full h-[350px] md:h-[550px] bg-cover bg-center back_img' style={{ backgroundImage: `url(${post.bg})` }} >

                        <div className='flex gap-2 p-4'>
                            <img className='rounded-full w-[60px] h-[60px] md:w-[75px] md:h-[75px] border-4 border-red-600 cursor-pointer'
                                src={post.profileImg} /> 
                            <div>
